...
## Emoties
# Anxiety, Aversion, Boredom, Distrust, Hope, Interest, Respect, Enthusiasm
d <- d %>% 
  mutate(aversion = recode(H4,
                        `Aversion` = 1,
                        .default = 0),
         anxiety = recode(H4,
                          `Anxiety` = 1,
...
tmp <- d %>% 
  group_by(id) %>% 
  summarise(anx_tot = sum(anxiety),
            aversion_tot = sum(aversion),
            boredom_tot = sum(boredom),
            distrust_tot = sum(distrust),
            enthusiasm_tot = sum(enthusiasm),
            hope_tot = sum(hope),
            interest_tot = sum(interest),
            respect_tot = sum(respect),
            n_id = n()) %>% 
  mutate(anxiety_tot = round(anx_tot/n_id,2),
         aversion_tot = round(aversion_tot/n_id,2),
  ...
  select(id, anxiety_tot, aversion_tot:respect_tot)
...
df <- d %>% 
  ...
  select(id, wave, weights, #survey info
         ...
         aversion:respect_tot, #emotions
         anxiety_tot:respect_tot,
